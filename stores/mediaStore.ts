...
export const useMediaStore = create<MediaStore>((set) => ({
  ...
  // Persistence actions
  setPersistedMovies: (movies: PersistedMovieFile[]) =>
    set({ persistedMovies: movies }),
  setPersistedTvShows: (tvShows: PersistedTvFile[]) =>
    set({ persistedTvShows: tvShows }),

  addPersistedMovie: (movie: PersistedMovieFile) =>
    set((state) => ({
      persistedMovies: [
        ...state.persistedMovies.filter((m) => m.tmdbId !== movie.tmdbId),
        movie,
      ],
    })),

  addPersistedTvShow: (tvShow: PersistedTvFile) =>
    set((state) => ({
      persistedTvShows: [
        ...state.persistedTvShows.filter((t) => t.tmdbId !== tvShow.tmdbId),
        tvShow,
      ],
    })),

  removePersistedMovie: (tmdbId: number) =>
    set((state) => ({
      persistedMovies: state.persistedMovies.filter((m) => m.tmdbId !== tmdbId),
    })),

  removePersistedTvShow: (tmdbId: number) =>
    set((state) => ({
      persistedTvShows: state.persistedTvShows.filter(
        (t) => t.tmdbId !== tmdbId
      ),
    })),

  // Session actions
  addSessionMovie: (movie: any) =>
    set((state) => ({
      sessionMovies: [
        ...state.sessionMovies.filter((m) => m.tmdbId !== movie.tmdbId),
        movie,
      ],
    })),
