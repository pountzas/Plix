 ...
 */
export function createPersistedMovieFile(movieFile: MovieFile, userId: string): PersistedMovieFile {
  // Validate required fields
  if (!validateMovieFile(movieFile)) {
    throw new Error('Invalid movie file data: missing required fields')
  }

  // Clean the data by removing undefined values and converting to Firestore-compatible format
  const cleanData = Object.fromEntries(
    Object.entries(movieFile).filter(([_, value]) => value !== undefined)
  )
 ...
 * Cleans undefined values for Firestore compatibility
 */
export function createPersistedTvFile(tvFile: TvFile, userId: string): PersistedTvFile {
  // Validate required fields
  ...
  // Clean the data by removing undefined values and converting to Firestore-compatible format
  const cleanData = Object.fromEntries(
    Object.entries(tvFile).filter(([_, value]) => value !== undefined)
  )
 ...
 */
export async function loadUserMovies(userId: string): Promise<PersistedMovieFile[]> {
  try {
 ...
 */
export async function loadUserTvShows(userId: string): Promise<PersistedTvFile[]> {
  try {
    ...
    console.log(`Loaded ${tvShows.length} TV shows from Firestore`)
    return tvShows
  } catch (error) {
...
export async function saveMovieToUserCollection(
  ...
  try {
    ...
    const movieRef = doc(db, 'users', userId, 'movies', movieFile.tmdbId.toString())
...
export async function saveTvShowToUserCollection(
  ...
  try {
    ...
    const tvShowRef = doc(db, 'users', userId, 'tvshows', tvFile.tmdbId.toString())
...
export async function saveMoviesBatchToUserCollection(
  ...
  try {
    ...
    console.log(`Saving ${validMovies.length} valid movies to Firestore (filtered ${movieFiles.length - validMovies.length} invalid)`)
...
export async function saveTvShowsBatchToUserCollection(
  ...
  try {
    ...
    console.log(`Saving ${validTvShows.length} valid TV shows to Firestore (filtered ${tvFiles.length - validTvShows.length} invalid)`)
    ...
    await batch.commit()
    console.log(`Successfully saved ${validTvShows.length} TV shows to Firestore`)
  } catch (error) {
